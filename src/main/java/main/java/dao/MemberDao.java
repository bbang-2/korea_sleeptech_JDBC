...
// 4) manager - MemberManager.java (실행 및 테스트 담당)

// MemberDao 클래스
// : Member 테이블에 대한 CRUD 작업 처리
...
//        String str = "Hello";
//        str += "JDBC"; // strd에 "JDBC를 추가한 것이 아니라! 기존 문자열은 버려지고 새로운 String 객체가 생성!
        ...
        // 마지막에 붙는 콤마와 공백을 제거
        // : 위에서 name 또는 email 중 최소 하나는 true이기 때문에
        //      , 마지막 두글자(', ")을 제거
        sql.deleteCharAt(sql.length() - 2); // 콤마까지 제거

        // WHERE 절 추가 (어떤 사용자 인지 특정 - id 기준)
        sql.append("WHERE id = ?");
